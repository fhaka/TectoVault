...
export function WorkHero({ hasPlaceholders }: { hasPlaceholders: boolean }) {
  ...
  return <section className="relative overflow-hidden bg-ink pb-20 pt-36 text-ink-foreground md:pb-24 md:pt-44" data-header-tone="dark">
    ...
    <div className="container-page relative"><p className="text-xs font-semibold uppercase tracking-[.28em] text-accent">{t("work.eyebrow")}</p><motion.h1 initial={reduceMotion ? false : { y: 60, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ duration: .8, ease: [0.16, 1, .3, 1] }} className="mt-5 max-w-4xl text-balance font-display text-5xl font-medium leading-[.94] md:text-7xl">{t("work.title")}</motion.h1><p className="mt-7 max-w-xl text-lg leading-relaxed text-ink-muted">{t("work.description")}</p>{hasPlaceholders ? <p className="mt-7 text-sm text-white/55">{t("work.samplesNote")}</p> : null}</div>
  </section>;
}
